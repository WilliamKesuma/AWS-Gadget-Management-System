...
from model import GetSignedHandoverFormResponse
...
def lambda_handler(event, context):
    try:
        ...
        # Query for latest Handover_Record
        ...
        # Check that the signed form exists
        signed_form_s3_key = handover_record.get("SignedFormS3Key")
        if not signed_form_s3_key:
            raise NotFoundException("Signed handover form is not available yet")

        # Generate presigned GET URL (60 min TTL)
        presigned_url = s3_client.generate_presigned_url(
            "get_object",
            Params={"Bucket": ASSETS_BUCKET, "Key": signed_form_s3_key},
            ExpiresIn=3600,
        )

        resp = GetSignedHandoverFormResponse(
            asset_id=asset_id,
