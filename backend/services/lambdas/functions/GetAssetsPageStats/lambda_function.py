...
from model import AssetsPageStatsResponse
...
def lambda_handler(event, context):
    try:
        ...
        response = AssetsPageStatsResponse(
            total_assets=int(item.get("TotalActiveAssets", 0)) if item else 0,
            in_stock=int(item.get("InStock", 0)) if item else 0,
            assigned=int(item.get("Assigned", 0)) if item else 0,
