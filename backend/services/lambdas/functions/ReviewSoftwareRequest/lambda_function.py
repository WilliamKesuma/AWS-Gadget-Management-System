...
from model import ReviewSoftwareRequestRequest, ReviewSoftwareRequestResponse
...
def lambda_handler(event, context):
    try:
        ...
        request = ReviewSoftwareRequestRequest(**body)
        ...
        # Verify the request is in a reviewable state
        if software_item["Status"] != Software_Status_Enum.PENDING_REVIEW:
            raise ConflictException("This request is not in a reviewable state")

        # Determine new status based on decision
        status_map = {
            "APPROVE": Software_Status_Enum.SOFTWARE_INSTALL_APPROVED,
            "ESCALATE": Software_Status_Enum.ESCALATED_TO_MANAGEMENT,
        ...
        if request.decision == "APPROVE":
            ...
            update_fields["RejectionReason"] = request.rejection_reason
        ...
        audit_log_kwargs = {
            ...
            "ActorID": actor_id,
            "Phase": "SOFTWARE_INSTALL_REVIEW",
            "PreviousStatus": Software_Status_Enum.PENDING_REVIEW,
        ...
        # Guard: status must still be PENDING_REVIEW at write time
        expr_names["#ExpectedStatus"] = "Status"
        expr_values[":expected_status"] = serializer.serialize(
            Software_Status_Enum.PENDING_REVIEW
        )
        ...
        response = ReviewSoftwareRequestResponse(
            asset_id=asset_id,
        ...
        return error("Software request status changed concurrently. Please retry.", 409)
